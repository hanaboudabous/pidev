...
//import tn.esprit.spring.repository.IData_santeRepo;
import tn.esprit.spring.repository.IDemandeContratRepo;
/*import Pidev.repository.Taux_InteretRepo;
import Pidev.repository.Type_santeRepo;
import Pidev.service.DataService;
...
public class ContratService {
	...
	String sante = "Sant�" ;
	String CapitalDiffere = "Capital diff�r�";
	String renteViagere = "Rente viag�re";
	String vieEntiere = "Vie enti�re";
	...
	public void calculeContrat(int id){
		...
		//double interet = 0.01*taux_Interet.findFirstByOrderByIdDesc().getTauxInteret();   /// le dernier interet  publi� par la bct
...
=======
			// scenario : si il va refus� la demande 
>>>>>>> branch 'master' of https://github.com/hanaboudabous/pidev.git
			//demandeContrat.deleteById(id);
	/*		String s = "votre demande est annul� , votre salaire ne peut pas supporter votre demande de capital assur� " ;
			d.setCause(s);
			d.setTraite(1);
			...
			if(d.getNomContrat().equals(CapitalDiffere)){  // choix du contrat
				if(d.getChoixPrime().equals(primeUnique)){ // choix type de prime 		
						if(d.getCapitalAssure() != 0){ // on a  ici calculer le capital demande et on va calcul� les primes
//	public double calculePUCapital_vieCapital(float capital ,int age , int annee, double interet , String sexe){
			...
			//////////////////////////////////////////// rente  Viagere /////////////////////////////////////////////
				...
				if(d.getChoixPrime().equals(primeUnique)){ // choix type de prime 		
					if(d.getCapitalAssure() != 0){ // on a  ici calculer le capital demande et on va calcul� les primes
//	public double calculePURenteillimteRente(float rente,int age ,  double interet , String sexe){ // � terme �chue)						
	...
	//	public double calculePURenteillimtePrime(float prime,int age ,  double interet , String sexe){ // � terme �chue)
						float primeP = (float) (d.getVal_prime()-d.getVal_prime()*frais) ;
...
//		public double calculePPVieEntiere(float prime ,float capital ,int age , int annee, double interet , String sexe ){// a terme d�avance
	...
	public float sommeSante(int i , int  iddemande){
		...
		// liste des contrats propos�s pour le client ... hethom yetl3ou 3and l client w houa mb3d yaccepti wela la ..
		@Transactional
